...
const mailProductsView = (products) => {
  const htmlTop = `
    <!DOCTYPE html>
    <head>
    <html lang="es">
                  ...
                  <td align="center" valign="top" style="font-size: 0; padding: 35px" bgcolor="#F44336">
                    ...
                    >
                      <table align="left" border="0" cellpadding="0" cellspacing="0" width="100%" style="max-width: 300px">
                        <tr>
                          <td align="left" valign="top" class="mobile-center" style="
                              font-family: Open Sans, Helvetica, Arial, sans-serif;
                              font-size: 36px;
                              font-weight: 800;
                              line-height: 48px;
  ...
  `.trim();
  const htmlProducts = products.map(product => htmlProduct(product)).join(" ");
  const total = products.map(e => +e.price).reduce((a, b) => a + b);
  const htmlBottom = `
                          </table>
                        </td>
                      </tr>
  ...
  `.trim();
  return `${htmlTop}${htmlProducts}${htmlBottom}`;
}
  ...
  <html lang="es">
    ...
    <body>
      ...
      <ul>
        ...
        <li><p><span style="font-weight: bold;">Password:</span> ${user.password}</p></li>
...
const renderMail = async (type, user, data) => {
  if(type == "users") return await fs.promises.writeFile(process.cwd() + `/response/${type}/${user}.html`, mailUserView(data));
  if(type == "carts") return await fs.promises.writeFile(process.cwd() + `/response/${type}/${user}.html`, mailProductsView(data));
};
